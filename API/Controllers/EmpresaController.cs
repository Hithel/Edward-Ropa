    ...
    public class EmpresaController : ApiBaseController
{
    ...
    {
        var entidad = await unitofwork.Empresa.GetAllAsync();
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<EmpresaDto>>> GetPaginacion([FromQuery] Params usuarioParams)
    {
        var entidad = await unitofwork.Empresa.GetAllAsync(usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
        var listEntidad = mapper.Map<List<EmpresaDto>>(entidad.registros);
        return new Pager<EmpresaDto>(listEntidad, entidad.totalRegistros, usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    }
    ...
    {
        var entidad = await unitofwork.Empresa.GetByIdAsync(id);
        if (entidad == null)
        {
            return NotFound();
        }
    ...
    [HttpPost]
        ...
        public async Task<ActionResult<Empresa>> Post(EmpresaDto entidadDto)
        {
            ...
            this.unitofwork.Empresa.Add(entidad);
    ...
    public async Task<ActionResult<EmpresaDto>> Put(int id, [FromBody] EmpresaDto entidadDto)
    {
        ...
        var entidad = this.mapper.Map<Empresa>(entidadDto);
        unitofwork.Empresa.Update(entidad);
        await unitofwork.SaveAsync();
        return entidadDto;
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        unitofwork.Empresa.Remove(entidad);
        await unitofwork.SaveAsync();
        return NoContent();
    }
}
