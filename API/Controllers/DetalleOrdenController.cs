    ...
    public class DetalleOrdenController : ApiBaseController
{
    ...
    {
        var entidad = await unitofwork.DetalleOrden.GetAllAsync();
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<DetalleOrdenDto>>> GetPaginacion([FromQuery] Params usuarioParams)
    {
        var entidad = await unitofwork.DetalleOrden.GetAllAsync(usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
        var listEntidad = mapper.Map<List<DetalleOrdenDto>>(entidad.registros);
        return new Pager<DetalleOrdenDto>(listEntidad, entidad.totalRegistros, usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    }
    ...
    {
        var entidad = await unitofwork.DetalleOrden.GetByIdAsync(id);
        if (entidad == null)
        {
            return NotFound();
        }
    ...
    public async Task<ActionResult<DetalleOrdenDto>> Put(int id, [FromBody] DetalleOrdenDto entidadDto)
    {
        ...
        var entidad = this.mapper.Map<DetalleOrden>(entidadDto);
        unitofwork.DetalleOrden.Update(entidad);
        await unitofwork.SaveAsync();
        return entidadDto;
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        unitofwork.DetalleOrden.Remove(entidad);
        await unitofwork.SaveAsync();
        return NoContent();
    }
}
