    ...
    public class MunicipioController : ApiBaseController
{
    ...
    {
        var entidad = await unitofwork.Municipio.GetAllAsync();
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<MunicipioDto>>> GetPaginacion([FromQuery] Params usuarioParams)
    {
        var entidad = await unitofwork.Municipio.GetAllAsync(usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
        var listEntidad = mapper.Map<List<MunicipioDto>>(entidad.registros);
        return new Pager<MunicipioDto>(listEntidad, entidad.totalRegistros, usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    }
    ...
    {
        var entidad = await unitofwork.Municipio.GetByIdAsync(id);
        if (entidad == null)
        {
            return NotFound();
        }
    ...
    [HttpPost]
        ...
        public async Task<ActionResult<Municipio>> Post(MunicipioDto entidadDto)
        {
            ...
            this.unitofwork.Municipio.Add(entidad);
    ...
    public async Task<ActionResult<MunicipioDto>> Put(int id, [FromBody] MunicipioDto entidadDto)
    {
        ...
        var entidad = this.mapper.Map<Municipio>(entidadDto);
        unitofwork.Municipio.Update(entidad);
        await unitofwork.SaveAsync();
        return entidadDto;
    }
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        unitofwork.Municipio.Remove(entidad);
        await unitofwork.SaveAsync();
        return NoContent();
    }
}
